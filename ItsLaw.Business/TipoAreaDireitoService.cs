...
{
    public class TipoAreaDireitoService : ITipoAreaDireito
    {
        private readonly TipoAreaDireitoRepository _repositorio = new TipoAreaDireitoRepository();
        private readonly UsuarioRepository  _UsuarioID   = new UsuarioRepository();
        protected                ItsLawContext _contexto = new ItsLawContext();


        public void TipoAreaDireitoAdd(TipoAreaDireito itens)
        {
            _repositorio.Add(itens);
        }

        public TipoAreaDireito TipoAreaDireitoGetById(int id)
        {
            var saida = _repositorio.GetById(id);
            return saida;
            }

        public IEnumerable<TipoAreaDireito> TipoAreaDireitoListagem(int pIDMaster)
        {
            //EGS 30.07.2018 - Incluindo nome dos usuarios de inclusao e manutencao FUNCIONANDO
            var tabela = (from ifs      in _contexto.TipoAreaDireito 
                          ...
                          select new
                          {
                              ...
                              IdTipoAreaDireito       = ifs.IdTipoAreaDireito,
                              ...
                              Ativo                   = ifs.Ativo
                          }).ToList().Select(x => new TipoAreaDireito()
                          {
                              IdMaster                = x.IdMaster,
                              IdTipoAreaDireito       = x.IdTipoAreaDireito,
        ...
        public IEnumerable<TipoAreaDireito> TipoAreaDireitoPesquisar(Expression<Func<TipoAreaDireito, bool>> filtro)
        {
            var saida = _repositorio.Find(filtro);
            return saida;
        }

        public void TipoAreaDireitoUpdate(TipoAreaDireito itens)
        {
