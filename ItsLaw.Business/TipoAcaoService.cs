...
{
    public class TipoAcaoService : ITipoAcao
    {
        private readonly TipoAcaoRepository _repositorio = new TipoAcaoRepository();
        ...
        public TipoAcao TipoAcaoGetById(int id)
        {
            var saida = _repositorio.GetById(id);
            return saida;
            }

        public IEnumerable<TipoAcao> TipoAcaoListagem(int pIDMaster)
        {
            //EGS 30.07.2018 - Incluindo nome dos usuarios de inclusao e manutencao FUNCIONANDO
            var tabela = (from ifs      in _contexto.TipoAcao 
                          ...
                          select new
                          {
                              ...
                              Ativo                   = ifs.Ativo
                          }).ToList().Select(x => new TipoAcao()
                          {
                              IdMaster                = x.IdMaster,
                              IdTipoAcao              = x.IdTipoAcao,
        ...
        public IEnumerable<TipoAcao> TipoAcaoPesquisar(Expression<Func<TipoAcao, bool>> filtro)
        {
