...
namespace ItsLaw.Infra.Mappings
{
    ...
    public partial class TestemunhaConfiguration : System.Data.Entity.ModelConfiguration.EntityTypeConfiguration<Testemunha>
    {
        public TestemunhaConfiguration()
            : this("dbo")
        {
        }

        public TestemunhaConfiguration(string schema)
        {
            ToTable("TESTEMUNHA", schema);
            HasKey(x => x.IdTestemunha);

            Property(x => x.IdTestemunha             ).HasColumnName(@"IDTESTEMUNHA"         ).HasColumnType("int"     ).IsRequired().HasDatabaseGeneratedOption(System.ComponentModel.DataAnnotations.Schema.DatabaseGeneratedOption.Identity);
